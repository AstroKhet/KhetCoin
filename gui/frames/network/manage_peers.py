...
class ManagePeersFrame(tk.Frame):
    ...
    def _config_peer_details(self):
        ...
        details_map = {
            ...
            "Last Block": f"{peer.last_block_ago}s" if peer.last_block_ago else "Never",
            "Last Transaction": f"{peer.last_tx_ago}s" if peer.last_tx_ago else "Never",
            "Last Send": f"{peer.last_send_ago}s" if peer.last_send_ago else "Never",
            "Last Receive": f"{peer.last_recv_ago}s" if peer.last_recv_ago else "Never",
